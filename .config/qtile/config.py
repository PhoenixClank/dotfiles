...
keys = [
	...
	Key([launch], 's', lazy.spawn('signal-desktop'), desc="show the [S]ignal window"),
...
screens = [
	Screen(
		left=Bar([
			...
			widget.Image(filename='/usr/lib/signal-desktop/resources/app/images/signal-logo.svg', margin=4, mouse_callbacks={'Button1': lambda: qtile.cmd_spawn('signal-desktop')}),
			widget.StatusNotifier(padding=4),
			mywidgets.vertical_stacking(widget.CurrentLayout),
		], 24, background=['#000000bf', '#0000003f']),
		wallpaper=random.choice(glob("Pictures/fanart/**/* (???? × ????).png", recursive=True)),
		#wallpaper="Pictures/LEGO/Exo-Force × Overwatch (2544 × 1431).png",
		wallpaper_mode='fill',
	),
	Screen(
		right=Bar([
